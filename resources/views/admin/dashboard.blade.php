        ...
        <div class="page-header au">
            <div class="d-flex justify-content-between align-items-start flex-wrap gap-4">
                ...
                <div class="header-actions align-self-center">
                    <a href="{{ route('booking.walkin') }}" class="btn-hdr btn-hdr-red">
                        <i class="fas fa-user-plus"></i> Walk-In
                    </a>
                    <a href="{{ route('advisor.create') }}" class="btn-hdr btn-hdr-ghost">
                        <i class="fas fa-wrench"></i> Proses Service
                    </a>
                    <a href="{{ route('booking.index') }}" class="btn-hdr btn-hdr-ghost">
        ...
        <div class="db-grid au d4">
            ...
            <div class="side-col">
                ...
                <div class="panel">
                    ...
                    <div class="qa-grid">
                        <a href="{{ route('booking.walkin') }}" class="qa-btn qa-red">
                            <i class="fas fa-user-plus"></i> Walk-In
                        </a>
                ...
                <div class="panel">
                    ...
                    <div class="svc-list">
                        @forelse($topServices as $svc)
                            @php $maxVal = $topServices->first()?->total ?? 1; @endphp
                            @php $pct = $maxVal > 0 ? round(($svc->total / $maxVal) * 100) : 0; @endphp
                            <div class="svc-item">
                                <div class="svc-row">
        ...
        <div class="bottom-grid au d6">
            ...
            <div class="panel">
                <div class="panel-hdr">
                    ...
                    <a href="{{ route('customers.index') }}" class="panel-link">Lihat semua</a>
                </div>
                <div class="tc-list">
                    @forelse($topCustomers as $i => $cust)
                        <div class="tc-item">
                            <div class="tc-rank {{ $i === 0 ? 'tr-1' : ($i === 1 ? 'tr-2' : ($i === 2 ? 'tr-3' : 'tr-n')) }}">
                                {{ $i + 1 }}
                            </div>
                            <div class="tc-avatar">{{ strtoupper(substr($cust->customer_name ?? 'X', 0, 1)) }}</div>
                            <div class="tc-info">
                                <div class="tc-name">{{ $cust->customer_name }}</div>
                                <div class="tc-wa">
            ...
            <div class="panel">
                <div class="panel-hdr">
                    ...
                    <a href="{{ route('inventory.index') }}" class="panel-link">Kelola</a>
                </div>
                <div class="ls-list">
                    @forelse($lowStockItems as $item)
                        <div class="ls-item">
                            <div class="ls-left">
                                <div class="ls-name">{{ $item->nama_barang }}</div>
                                <div class="ls-price">Jual: Rp {{ number_format($item->harga_jual, 0, ',', '.') }}</div>
                            </div>
                            @if($item->jumlah_barang == 0)
                                <span class="ls-badge-red"><i class="fas fa-times me-1"></i>HABIS</span>
                            @elseif($item->jumlah_barang <= 3)
                                <span class="ls-badge-red">{{ $item->jumlah_barang }} unit</span>
                            @else
                                <span class="ls-badge-warn">{{ $item->jumlah_barang }} unit</span>
                            @endif
                        </div>
                    @empty
                        <div class="empty-box">
            ...
            <div class="panel">
                <div class="panel-hdr">
                    <div class="panel-title">
                        ...
                        Antrian Hari Ini
                        <span class="panel-badge-red">{{ $queueBookings->count() }} aktif</span>
                    </div>
                    <a href="{{ route('booking.index') }}" class="panel-link">Kelola semua</a>
                </div>
                            ...
                            @forelse($queueBookings as $booking)
                                ...
                                <tr>
                                    <td class="text-center">
                                        <div class="q-badge">{{ $booking->queue_number }}</div>
                                    </td>
                                    <td>
                                        <div class="q-name">{{ Str::limit($booking->customer_name, 15) }}</div>
                                        <div class="mt-1">
                                            <span class="q-plate">{{ strtoupper($booking->plate_number) }}</span>
                                            <span class="q-type ms-1">{{ ucfirst($booking->vehicle_type) }}</span>
                                        </div>
                                    </td>
